...
namespace CE::Vulkan
{
	...
	VulkanRenderPass::VulkanRenderPass(Device* device, const VulkanRenderPass::Descriptor& descriptor)
		: device(device)
		, desc(descriptor)
	{
		...
		for (const VulkanRenderPass::AttachmentBinding& attachmentBinding : desc.attachments)
		{
		...
		for (const VulkanRenderPass::SubPassDescriptor& subpassDesc : desc.subpasses)
		{
			VkSubpassDescription subpass{};
			subpass.pipelineBindPoint = VK_PIPELINE_BIND_POINT_GRAPHICS;
			
			for (const VulkanRenderPass::SubPassAttachment& subpassAttachment : subpassDesc.colorAttachments)
			{
			...
			for (const VulkanRenderPass::SubPassAttachment& subpassAttachment : subpassDesc.resolveAttachments)
			{
				VkAttachmentReference ref{};
				ref.attachment = subpassAttachment.attachmentIndex;
				ref.layout = subpassAttachment.imageLayout;
			...
			for (const VulkanRenderPass::SubPassAttachment& subpassAttachment : subpassDesc.subpassInputAttachments)
			{
				VkAttachmentReference ref{};
				ref.attachment = subpassAttachment.attachmentIndex;
				ref.layout = subpassAttachment.imageLayout;
	...
	void VulkanRenderPass::BuildDescriptor(Vulkan::Scope* pass, VulkanRenderPass::Descriptor& outDescriptor)
	{
		outDescriptor = {};
		if (pass == nullptr)
			return;
	...
	void VulkanRenderPass::BuildDescriptor(const RHI::RenderPassLayout& rpLayout, Descriptor& outDescriptor)
	{
		outDescriptor = {};
	...
	void VulkanRenderPass::Descriptor::CompileDependencies()
	{
		dependencies.Clear();
	...
	RHI::ScopeAttachmentUsage VulkanRenderPass::GetAttachmentUsage(u32 attachmentIndex)
	{
	...
	SIZE_T VulkanRenderPass::Descriptor::GetHash() const
	{
		SIZE_T hash = 0;
	...
	SIZE_T VulkanRenderPass::SubPassDescriptor::GetHash() const
	{
		SIZE_T hash = 0;
	...
	SIZE_T VulkanRenderPass::SubPassAttachment::GetHash() const
	{
		if (!IsValid())
			return 0;
	...
	SIZE_T VulkanRenderPass::AttachmentBinding::GetHash() const
	{
